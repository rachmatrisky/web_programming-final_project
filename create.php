<?php
  ...
  $item_code = $item_name = $item_price = $item_stock = $item_img = $item_desc = $item_ctg = "";


  $item_code_err = $item_name_err = $item_price_err = $item_stock_err = $item_img_err = $item_desc_err = $item_ctg_err = "";
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!file_exists($input_item_img["tmp_name"])) {
      $item_img_err = "Tolong masukkan file gambar";
    } else {
    ...
    if (empty($item_code_err) && empty($item_name_err) && empty($item_price_err) && empty($item_stock_err) && empty($item_img_err) && empty($item_desc_err) && empty($item_ctg_err)) {
      ...
      $sql = "INSERT INTO barang (kode_barang, nama_barang, harga, stok, gambar, deskripsi, kategori)
      VALUES ('$item_code', '$item_name', '$item_price', '$item_stock', '$item_img', '$item_desc', '$item_ctg')";
...
<header>
  ...
  <main class="my-5">
    <h1 class="text-center my-3">Menambahkan Data Barang</h1>
    <form action="<?= htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="post" class="form-container container d-flex flex-column justify-content-center gap-2 py-2 border rounded-3" enctype="multipart/form-data">
      <div class="p-2">
      ...
      <div class="p-2">
        <label for="inputImg" class="form-label">Gambar</label>
        <input type="file" name="inputImg" id="inputImg" class="form-control <?= (!empty($item_img_err)) ? "is-invalid" : ""; ?>" value="<?= $item_img; ?>">
        <div class="invalid-feedback">
          <?= $item_img_err ?>
        </div>
      </div>
